...
const draftsController = require(path.join(__dirname, 'controllers/DraftsController'));
const votesController = require(path.join(__dirname, 'controllers/VotesController'));
...
const draftSchema = JSON.parse(fs.readFileSync(path.join('.', 'json_schemas', 'draft_schema.json')).toString());
const voteSchema = JSON.parse(fs.readFileSync(path.join('.', 'json_schemas', 'vote_schema.json')).toString());
var validator = new Validator({ allErrors: true });
validator.ajv.addSchema([userSchema, filmSchema, reviewSchema, draftSchema, voteSchema]);
...
var expressAppConfig = oas3Tools.expressAppConfig(path.join(__dirname, 'api/openapi.json'), options);
...
app.post('/api/films/public/:filmId/reviews/:reviewId/drafts', isLoggedIn, validate({ body: draftSchema }), draftsController.createDraft);
app.get('/api/films/public/:filmId/reviews/:reviewId/drafts', isLoggedIn, draftsController.getDraftsByReview);
app.get('/api/films/public/:filmId/reviews/:reviewId/drafts/:draftId', isLoggedIn, draftsController.getDraftById);
app.post('/api/films/public/:filmId/reviews/:reviewId/drafts/:draftId/votes', isLoggedIn, validate({ body: voteSchema }),  votesController.createVote);
app.get('/api/films/public/:filmId/reviews/:reviewId/drafts/:draftId/votes', isLoggedIn, votesController.getVotesByDraft);
app.get('/api/films/public/:filmId/reviews/:reviewId/drafts/:draftId/votes/:voteId', isLoggedIn, votesController.getVoteById);
