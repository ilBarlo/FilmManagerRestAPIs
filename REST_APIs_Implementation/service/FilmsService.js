...
exports.getInvitedFilms = function (req) {
    return new Promise((resolve, reject) => {
        ...
        var sql = "SELECT f.id as fid, f.title, f.owner, f.private, f.watchDate, f.rating, f.favorite, c.total_rows FROM films f, reviews r, reviews_users rs, (SELECT count(*) total_rows FROM films f2, reviews r2, reviews_users rs2 WHERE  f2.private = 0 AND f2.id = r2.filmId AND r2.id = rs2.reviewId AND rs2.reviewerId = ?) c WHERE  f.private = 0 AND f.id = r.filmId AND r.id = rs.reviewId AND rs.reviewerId = ?"
        var limits = getPagination(req);
        if (limits.length != 0) sql = sql + " LIMIT ?,?";
...
exports.getInvitedFilmsTotal = function (reviewerId) {
    return new Promise((resolve, reject) => {
        var sqlNumOfFilms = "SELECT count(*) total FROM films f, reviews r, reviews_users rs WHERE  f.private = 0 AND f.id = r.filmId AND r.id = rs.reviewId AND rs.reviewerId = ? ";
