  ...
  if(differentFilm){
    ...
    .catch(function (response) {
      if(response === '403A'){
        ...
        utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The user has already been assigned to a single review' }], }, 403);
      }
      else if (response == 404){
...
module.exports.updateSingleReview = function updateSingleReview (req, res, next) {
  ...
  if(req.body.completed == undefined) {
    ...
    Reviews.updateSingleReview(req.body, req.params.reviewId, 0) /* 0 to check that is a single review */
    .then(function(response) {
        utils.writeJson(res, response, 204);
    })
    .catch(function(response) {
        if(response == 403){
            utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The review is cooperative.' }], }, 403);
        }
        else if (response == 404){
            utils.writeJson(res, { errors: [{ 'param': 'Server', 'msg': 'The review does not exist.' }], }, 404);
        }
